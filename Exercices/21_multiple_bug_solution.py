...
def main():
    ...
    class Auction(sp.Contract):
        ...
        #def award_tez(self, destination, amount):
            #error with method
            #if not self.data.tez_to_claim.contains(destination):
             #   self.data.tez_to_claim[destination] = sp.tez(0)
            #self.data.tez_to_claim[destination] += amount

        @sp.entrypoint
        def mint(self, metadata):
            assert sp.amount == sp.tez(1)
            #self.award_tez(self.data.admin, sp.amount)
        ...
        def bid(self, itemID):
            ...
            #self.award_tez(item.topBidder, item.topBid)
