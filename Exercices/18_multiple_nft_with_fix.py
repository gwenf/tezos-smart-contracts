...
class NftMarketplace(sp.Contract):
    def __init__(self, fee_rate, admin ):
        self.init(
            ...
            tokens = sp.big_map(
                {},
                tkey = sp.TPair(sp.TContract, sp.TInt),
                tvalue = sp.TRecord
            ),
            ledger = sp.big_map({}, tkey = sp.TAddress, tvalue = sp.TTez) 
        )
    ...
    def removeFromMarketplace(self, contract, tokenID):
        ...
        sp.verify(sp.sender == self.data.tokens[(contract, token_id)].owner, 'not your property')
    ...
    def buy(self, token_id):
        ...
        sp.verify(sp.amount == self.data.tokens[token_id].price)
    ...
    def test():
        ...
        c1 = NftMarketplace(admin=alice, fee_rate=sp.nat(3))
