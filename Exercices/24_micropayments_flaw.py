    ...
    class Micropayments(sp.Contract):
        ...
        def deposit(self, destination, deadline, public_key):
            ...
            if not self.data.deposits.contains(key):
                self.data.deposits[key] = sp.record(deadline=deadline, amount=sp.amount, public_key = public_key)
            else:
        ...
        def claimPayments(self, source, packed_message, signature):
            ...
            message = sp.unpack(packed_message, sp.record(destination = sp.address, amount = sp.mutez)).unwrap_some()
