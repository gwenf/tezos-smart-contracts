...
def main():
    ...
    class Membership(sp.Contract):
        ...
        def join_with_tokens(self, nbTokens):
            ...
            assert sp.mul(token_price, nbTokens) > self.data.membership_price
    ...
    class Attacker(sp.Contract):
        def __init__(self, ledger, liquidity_pool, membership, flashLoan, membership_price):
            self.data.ledger = ledger
            self.data.liquidity_pool = liquidity_pool
        ...
        def attack_part3(self):
            ...
            priceInTokens = sp.fst(sp.ediv(self.data.membership_price, token_price).unwrap_some()) + 1
            ...
            sp.transfer((), amount_repaid, flashLoan_repay)
...
def test():
    ...
    attacker = main.Attacker(ledger = ledger.address, liquidity_pool = liquidity_pool.address, membership = membership.address, flashLoan = flashLoan.address, membership_price = sp.tez(1000))
    scenario += attacker
